...
- [~] **3.7 Phase 2–3 audit fix-pack** 🤖 *(from the codex gpt-5.6-sol xhigh audit, `docs/review/2026-07-26-phase-2-3-audit-codex.md` — criteria recorded before build per protocol. Scoping: P1-5 full deadline/settle machinery stays 4.1 (a claim-side 20-min age gate + attempt ceiling land HERE) · P1-7 durable outbox/watchdog stays 4.2 (duplicate-delivery re-interpretation lands HERE) · P2-5 DST-instant math and P2-2 canonical med SKU deferred post-hackathon, stated risk accepted (spring-forward is Mar 2027; med list is curated with form in the display name).*
  - [ ] 3.7.1 **Mode isolation (P1-1)**: `searches.dial_mode` (DEV_TEST/REAL/DEMO; legacy rows backfilled DEV_TEST); `create_search` filters the candidate pool by mode (REAL → verified, non-dev_test only) and scopes the verdict cache to same-mode searches; demo searches write DEMO, are never claimable, and their rows don't count toward the global cap; claim adds `s.dial_mode = p_dial_mode` → passes when: `mode.isolation` int test — a REAL search over mixed pharmacies targets ONLY verified real ones, copies only REAL verdicts; a DEMO dialing row doesn't eat cap; a DEV_TEST search never claims in REAL mode.
  - [ ] 3.7.2 **Claim-side politeness hardening (P1-2 + P1-5 + P1-6)**: fail-closed SQL hours validation + 60-min stay-open check inside the locked claim; DB-side `least(p_global_cap, 8)` / `least(p_per_search_cap, 3)`; per-search total attempt ceiling (12); claims refused once the search is >20 min old → passes when: `claim.stay-open` (open-but-closing-in-30 and junk-hours rows are never claimed) + `caps.clamped` (cap=20 request still claims ≤8) + attempt-ceiling and age-gate tests green.
  - [ ] 3.7.3 **Atomic bench promotion + settle (P1-3)**: `promote_bench` / `settle_if_drained` become advisory-locked SQL RPCs (single-row `FOR UPDATE SKIP LOCKED` promotion) used by record_call_event, dispatch-failure, and extract_result paths; extraction EXHAUSTION now promotes too → passes when: `bench.exhaustion-promotes` green; concurrent double-terminal test promotes exactly one replacement per dead call and never double-settles.
  - [ ] 3.7.4 **Webhook duplicate re-interpretation (P1-7 minimal)**: a duplicate raw insert (23505) still schedules idempotent interpretation, so a provider redelivery heals a lost `after()` → passes when: `webhook.duplicate-interprets` — call stuck `dialing`, duplicate delivery arrives, transition applies exactly once.
  - [ ] 3.7.5 **Client payload hygiene (P1-4 + P2-2 parser)**: verdict jsonb allowlisted to structured fields only — notable quotes and verbatim excerpts never reach client-granted columns (raw stays in call_events; extraction stays re-runnable); `parseQuantity` unit-aware + conservative (word-numbers with units OK; no `few→3` fabrication; never grabs strength digits like "the 25,000") → passes when: `verdict.no-verbatim` + `quantity.conservative` green; presentation mapper tests still green.
  - [ ] 3.7.6 **DB bucket/payload matrix (P2-3)**: CHECK matrix — bucket 1↔in_stock, 2↔orderable, 3↔out_of_stock, 4/null↔no stock claim (`NOT VALID` on cloud to tolerate legacy rows; enforced for all new writes); prove-constraints extended → passes when: proof script rejects every mismatch (13 → 17+).
  - [ ] 3.7.7 **Portfolio + eventless stalls (P2-4 + P2-1)**: same-chain supermarket swap satisfies the quota when legal; `create_search` immediately settles an all-cached search (bench expired, search complete); a definite-rejection wave triggers ONE bounded re-claim pass (no hot loop) → passes when: `portfolio.supermarket-swap` + all-cached-settles + rejection-repass tests green.
  - **Step success:** typecheck + full suite + build green · local `db reset` + extended proof green · migration pushed to cloud + prod redeployed · cloud REAL-mode pool provably excludes dev_test pharmacies (SQL check against prod).
