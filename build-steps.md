...
- [x] **4.2 Watchdog (pg_cron)** 🤖 ✅ — 3 rules: stale in-flight → reconcile via `GET /conversations/{id}`; stuck `transcript_ready` → re-extract; dead-quiet search → settle (deadline sweep). Anomaly log on every action → passes when: `watchdog.reconcile`: a deliberately-dropped webhook is rescued < 90s (command-level with injected provider client; cron cadence 60s), a stuck transcript re-extracts, actions logged. ✅ 6/6 watchdog + 3/3 `internal.secret-guard` tests: lost-transcription replayed through recordCallEvent (transcript stored, extract fired) · 404 → unreached + bench promotion · in-progress/ambiguous touch NOTHING · young rows never looked up · stuck re-extracts, fresh doesn't · sweep + drained-active both settle. *(Design as planned: `lib/commands/watchdog.ts` + `POST /api/internal/watchdog` (INTERNAL_SECRET, fail-closed) + `scripts/setup-watchdog.sql` (pg_cron+pg_net, run-by-hand — secret can't live in a migration). DEPLOY DEPENDS: cloud `supabase db push` (4.1 migration) + Vercel `INTERNAL_SECRET` env + running setup-watchdog.sql vs cloud — queued for the phase-end handoff.)*
- [?] **4.3 Anonymous auth + RLS** 🤖 + 🧑 — `signInAnonymously`, `owner` scoping, transcripts excluded from client grants → passes when: 🤖 `rls.two-sessions`: session B cannot select/subscribe to session A's rows ✅ (3/3, run twice: owner positive control + transcript/extraction 42501 even for the owner + stranger selects zero rows + stranger's realtime subscription silent through repeated board ticks); 🧑 quick two-browser check confirms — **handoff queued for Marvin at phase end**.
- [~] **4.4 Abuse guards** 🤖 — 1 active search per session (owner uid — IP-level guard dropped, stated: the anonymous session IS the identity and serverless IP state adds nothing today) · `INTERNAL_SECRET` on internal routes · `DIALING_ENABLED` kill switch inside the claim path → passes when: scripted 2nd search → rejected (409 + the existing searchId, so the client resumes instead of duplicating); internal route w/o secret → 401; switch OFF → dispatch claims nothing (existing kill-switch test in dispatch.stress).
