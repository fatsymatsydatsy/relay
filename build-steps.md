...
- [~] **3.4 extract_result** 🤖 + 🧑 — gpt-5.4-mini, strict schema, 2 retries → escalate → `extraction_failed`; verdict fan-out; rank keys → 🤖 ✅ integration replay of 5 transcript shapes (partial-stock b1 · orderable b2 · plain-no+shortage b3 · wrong-branch b4-no-payload · voicemail→unreached) all land correctly; escalation ladder gpt-5.4-mini ×2 → gpt-5.6-sol proven; total failure = honest `extraction_failed` b4 + anomaly; fan-out copies to same-pharmacy+med queued calls; re-run = noop. `scripts/replay-transcripts.mjs` = the real-LLM evidence tool. 80/80. → 🧑 pending: Marvin reads 5 REAL transcript→verdict pairs (produced by the 3.5/3.6 role-plays; run the replay script on them).
