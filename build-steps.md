...
- [~] **4.2 Watchdog (pg_cron)** 🤖 — 3 rules: stale in-flight → reconcile via `GET /conversations/{id}`; stuck `transcript_ready` → re-extract; dead-quiet search → settle (deadline sweep). Anomaly log on every action → passes when: `watchdog.reconcile`: a deliberately-dropped webhook is rescued < 90s (command-level with injected provider client; cron cadence 60s), a stuck transcript re-extracts, actions logged. *(Design: `lib/commands/watchdog.ts` + `POST /api/internal/watchdog` guarded by INTERNAL_SECRET (4.4's guard lands early); pg_cron + pg_net call the route every 60s — job created by `scripts/setup-watchdog.sql` run-by-hand since the secret can't live in a committed migration. Reconcile is fail-safe: only a DEFINITE provider answer transitions a row — done+transcript → interpret; not-found after grace → unreached+promote; anything ambiguous → anomaly only, number stays politely blocked.)*
