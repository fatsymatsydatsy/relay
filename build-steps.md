...
- [~] **1.5 Realtime proof** 🤖 + 🧑 *(was 1.4)* — anonymous sign-in, owner-scoped subscribe to `searches`+`calls`, stub `create_search` command writes fixture rows (per-caller, no cloud pre-seed needed); `/search?engine=live` selects the live engine; manually UPDATE a row in SQL → passes when: 🤖 scripted check (`scripts/test-realtime.mjs`) shows an UPDATE land on a subscribed client < 2s; 🧑 Marvin watches the board update < 2s with no refresh.
