...
UI reads exactly two live feeds via Supabase realtime: the `searches` row + its `calls` rows (RLS scopes to the anonymous session's own searches). Per call: pharmacy name/address/distance · status · rank bucket 1–4 · verdict jsonb · timestamps. Sort: bucket, then ETA/distance. States to design: queued · dialing · 3 verdict kinds · couldn't-reach · expired. The two gated hard rules (build-steps 1.4 checks exactly these; renumbered in the 26 Jul Phase-1 restructure): disclaimer always visible ("MedFind checks pharmacy stock availability. It does not provide medical advice… 999/111") · bucket 4 must NEVER look like a stock verdict. Also required: partial stock reads "in stock — 1 box (you need 2)"; raw transcripts never reach the client (enforced by column grants, not by the UI). Design tokens: DESIGN.md.
